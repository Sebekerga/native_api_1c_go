...
static AddIn *CreateAddIn()
{
    ...
    AddIn *addin = (AddIn *)malloc(sizeof(AddIn));
...
// for linux only
    ...
    void (*add_error)(AddIn *self, unsigned short code, const int16_t *source, const int16_t *description, long flag);
} ConnectionVTable;
...
// for linux only
    ...
    long int (*get_info)(AddIn *self);

    // `done` function pointer, used to free resources
    // gets reference to self
    void (*done)(AddIn *self);
} InitDoneBaseVTable;

static bool _init(AddIn *self, Connection *connection)
{
    logToConsole("Init function was called");
    return true;
}

static bool _set_mem_manager(AddIn *self, MemoryManager *mem_manager)
{
...
static long int _get_info(AddIn *self)
{
    logToConsole("Get info function was called");
    return 2000;
}

static void _done(AddIn *self)
{
...
// for linux only
    ...
    bool (*get_prop_val)(AddIn *self, long int prop_index, void *value);
    ...
    long int (*get_n_params)(AddIn *self, long int method_index);
    ...
    // returns true if value was got successfully
    bool (*get_param_default_val)(AddIn *self, long int method_index, long int param_index, void *value);
...
// for linux only
    ...
    void (*set_locale)(AddIn *self, const wchar_t *locale);

} LocaleBaseVTable;

static void _set_locale(AddIn *self, const wchar_t *locale)
{
...
// for linux only
    ...
    void (*set_user_language)(AddIn *self, const wchar_t *user_language);

} UserLanguageBaseVTable;

static void _set_user_language(AddIn *self, const wchar_t *user_language)
{
